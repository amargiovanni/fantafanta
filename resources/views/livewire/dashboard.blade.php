<div class="space-y-6">
    ...
    @if (session('dashboard'))
        <div class="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800 print:hidden dark:border-emerald-800 dark:bg-emerald-950 dark:text-emerald-400">
            {{ session('dashboard') }}
        </div>
    @endif

    @if (session('dashboard-error'))
        <div class="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 print:hidden dark:border-red-800 dark:bg-red-950 dark:text-red-400">
            {{ session('dashboard-error') }}
        </div>
    @endif

    <div class="grid grid-cols-1 gap-4 sm:grid-cols-4 print:hidden">
    ...
    <div class="rounded-lg border border-slate-200 bg-white print:border-0 print:rounded-none dark:border-slate-800 dark:bg-slate-900">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-4 print:hidden dark:border-slate-800">
            ...
            @if ($plan)
                <div class="flex items-center gap-3">
                    ...
                    <button
                        ...
                        class="print:hidden rounded-md border border-slate-300 px-2.5 py-1 text-xs font-medium text-slate-600 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
                    >
                        Stampa piano
                    </button>
                </div>
        ...
        {{-- Testata di sola stampa: la pagina stampata non ha la nav né i
             pulsanti, quindi versione e data vanno ripetute qui in chiaro. --}}
        @if ($plan)
            <div class="hidden border-b border-slate-200 px-5 py-3 text-sm print:block">
                <p class="font-semibold text-slate-900">Piano d'acquisto — versione {{ $plan->version }}</p>
                <p class="text-xs text-slate-600">
                    Generato {{ $plan->created_at->format('d/m/Y H:i') }} · stampato {{ now()->format('d/m/Y H:i') }}
                </p>
            </div>
        @endif
            ...
            <div class="divide-y divide-slate-100 dark:divide-slate-800">
                @foreach (['P' => 'Portieri', 'D' => 'Difensori', 'C' => 'Centrocampisti', 'A' => 'Attaccanti'] as $ruolo => $etichetta)
                    @if (! empty($planSlots[$ruolo]))
                        <div class="px-5 py-4 print:break-inside-avoid print:px-0 print:py-2">
                            <h3 class="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ $etichetta }}</h3>

                            <div class="mt-3 overflow-x-auto print:mt-1 print:overflow-visible">
                                <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800 print:text-xs">
                                    <thead class="text-left text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">
                                        <tr>
                                            <th class="w-10 py-2 pr-3 print:py-0.5">#</th>
                                            <th class="py-2 pr-3 print:py-0.5">Titolare</th>
                                            <th class="py-2 pr-3 print:py-0.5">Target</th>
                                            <th class="py-2 pr-3 print:py-0.5">Max</th>
                                            <th class="py-2 pr-3 print:py-0.5">Alternative</th>
                                            <th class="py-2 print:py-0.5">Stato</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                        @foreach ($planSlots[$ruolo] as $riga)
                                            ...
                                            <tr wire:key="slot-{{ $slot->id }}">
                                                <td class="py-2 pr-3 text-slate-400 print:py-0.5 dark:text-slate-500">{{ $slot->slot_index }}</td>
                                                <td class="py-2 pr-3 font-medium text-slate-900 print:py-0.5 dark:text-slate-100">
                                                    {{ $riga['player_name'] ?? 'nessuno disponibile' }}
                                                </td>
                                                <td class="py-2 pr-3 text-slate-700 print:py-0.5 dark:text-slate-300">{{ $slot->target_price }}</td>
                                                <td class="py-2 pr-3 text-slate-500 print:py-0.5 dark:text-slate-400">{{ $slot->max_price }}</td>
                                                <td class="py-2 pr-3 text-slate-500 print:py-0.5 dark:text-slate-400">
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 print:hidden">
